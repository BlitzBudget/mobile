...
void main() {
  AuthTokenLocalDataSourceImpl dataSource;
  SecureKeyValueStoreImpl mockSecureKeyValueStoreImpl;
  final authTokenCacheName = 'auth_token';
  final authTokenValue =
  ...
  group('Fetch AuthToken from Key Value Store', () {
    test(
      ...
      () async {
        // arrange
        when(mockSecureKeyValueStoreImpl.getString(key: authTokenCacheName))
            .thenAnswer((_) => Future.value(authTokenValue));
        // act
        final authTokenResult = await dataSource.readAuthToken();
        // assert
        verify(mockSecureKeyValueStoreImpl.getString(key: authTokenCacheName));
